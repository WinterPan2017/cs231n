...
Description: Copyright © 1999 - 2020 Winter. All Rights Reserved. 
             ...
             Finished Styletransfer-PyTorch.ipynb here.
             
Author: Winter
Email: 837950571@qq.com
Date: 2020-12-17 16:12:09
LastEditTime: 2020-12-20 16:46:26
'''
import torch
...
def gram_matrix(features, normalize=True):
    ...
    grams = []
    for i in range(N):
        gram = features[i].mm(features[i].T)
        grams.append(gram)
    gram = torch.cat(grams, 0)
    if normalize:
        gram /= H * W * C
    return gram
...
def style_transfer(content_image,
    ...
    content_img = preprocess(PIL.Image.open(content_image),
                             size=image_size).type(dtype)
    feats = extract_features(content_img, cnn)
    content_target = feats[content_layer].clone()

    # Extract features for the style image
    style_img = preprocess(PIL.Image.open(style_image),
                           size=style_size).type(dtype)
...
if __name__ == "__main__":
    ...
    student_output = content_loss(content_weight, c_feats[content_layer],
                                  feats[content_layer]).cpu().data.numpy()
